...
export default function WritingCommentsTab({ writing }: { writing: WritingApiDto }) {
    ...
    const handleSubmitComment = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (replyingTo) {
            const replyComment: ComicCommentInterface = {
                id: Date.now(),
                author: name.trim(),
            ...
            setComments((current) =>
                current.map((comment) => {
                    if (comment.id !== replyingTo.id) return comment;
                    return { ...comment, thread: [...(comment.thread ?? []), replyComment] };
                })
            );
        ...
        const createdComment: ComicCommentInterface = {
            id: Date.now(),
            author: name.trim(),
            comment: newComment.trim(),
            content: "0",
