...
async function getLatestNews(): Promise<LatestNewsItem[]> {
    try {
        const [artResponse, comicsResponse, writingResponse] = await Promise.all([
            fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Art/GetAll`, { cache: "no-store" }),
            fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Comic/GetAll`, { cache: "no-store" }),
            fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Writing/GetAll`, { cache: "no-store" }),
        ]);
