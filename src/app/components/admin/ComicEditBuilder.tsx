...
function ComicListView({
    ...
    async function deleteComic(comic: ComicApiDto) {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Comic/Delete/${comic.comicId}`, {
                method: "DELETE",
                credentials: "include",
            });
            if (!res.ok) {
...
export default function ComicEditBuilder({ onBack }: Props) {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Comic/GetAll`, { credentials: "include" })
            .then((res) => {
    ...
    async function handleSelect(comic: ComicApiDto) {
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL_DEV}/Api/Comic/Get/${comic.comicId}`,
                { credentials: "include" }
            );
